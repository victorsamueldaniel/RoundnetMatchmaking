...
from core.models import _SPEC_KEY_TO_ATTR
        ...
        for team, other in ((game.team_A, game.team_B), (game.team_B, game.team_A)):
            for player in team.players:
                ...
                has_same_teammate = any(
                    frozenset([player, mate]) in teammate_history
                    for mate in team.players
                )
                terms.append(("same_teammate", -float(weight * has_same_teammate)))

                repeated = sum(
                    1
                    for players_set in same_game_history
                    for q in players_set
                    if q in game.participants
                )
                divisor = _divisor(
                    p[
